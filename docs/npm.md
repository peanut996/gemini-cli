...
This monorepo contains two main packages: `@peanut996/gemini-cli` and `@peanut996/gemini-cli-core`.
...
When this package is published, it is bundled into a single executable file. This bundle includes all of the package's dependencies, including `@peanut996/gemini-cli-core`. This means that whether a user installs the package with `npm install -g @peanut996/gemini-cli` or runs it directly with `npx @peanut996/gemini-cli`, they are using this single, self-contained executable.
...
```bash
npm install -g @peanut996/gemini-cli@nightly
```
...
- `npx -y @peanut996/gemini-cli@latest --version` to validate the push worked as expected if you were not doing a rc or dev tag
- `npx -y @peanut996/gemini-cli@<release tag> --version` to validate the tag pushed appropriately
- _This is destructive locally_ `npm uninstall @peanut996/gemini-cli && npm uninstall -g @peanut996/gemini-cli && npm cache clean --force &&  npm install @peanut996/gemini-cli@<version>`
...
- **Simplified Script Execution**: You can run scripts in any package from the root of the project using the `--workspace` flag. For example, to run the `build` script in the `cli` package, you can run `npm run build --workspace @peanut996/gemini-cli`.
